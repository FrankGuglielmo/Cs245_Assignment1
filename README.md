...
ArrayList of integers. This allowed me to not only use less loops, but would make it poosible to use 
...
being sortedRegions.get() and sortedRegions.get(i + 1), and a right array, with the bounds being
sortedRegions.get(i + 2) and sortedRegions.get(i + 3). After getting the two arrays, I merged them into
arr like the old merge sort does. Once completed, I didn't need the ending index at 
sortedRegions.get(i + 1) or the starting index at sortedRegions.get(i + 2), so I removed them. This is 
because the new bounds of my sorted array started at sortedRegions.get(0) and ended at 
sortedRegions.get(3). I ran this same process on the rest of the sortedRegions in arr until the size of 
my sortedRegions Arraylist was less than 2, with sortedRegions.get(0) and sortedRegions.get(1) being the 
bounds of the original array, only this time it was sorted.
